...
class ScancodeOther extends StatefulWidget {
  const ScancodeOther({Key key}) : super(key: key);

  @override
  State<StatefulWidget> createState() => _ScancodeOtherState();
}

class _ScancodeOtherState extends State<ScancodeOther> {
  Barcode result;
        ...
        child: Scaffold(
          appBar: AppBar(
            title: Text(
              'SCAN CODE',
              style: TextStyle(
                  color: Colors.black, fontFamily: 'Kanit', fontSize: 18.0),
            ),
  ...
  Future<dynamic> fetchAlbum() async {
    ...
    if (response.statusCode == 200) {
      ...
      if (jsonDecode(response.body)['data'] == '' ||
          ...
          jsonDecode(response.body)['data'] == 'null') {
        print('sssss');
        Navigator.pushNamed(context, '/noproductother');
      } else {
        print('scan done');
                              ...
                              Container(
                                ...
                                child: SizedBox(
                                  ...
                                  child: ElevatedButton(
                                    style: ElevatedButton.styleFrom(
                                        primary: Colors.blue),
                                    child: Text('นับสต็อก(หลายชิ้น)', style: TextStyle(fontFamily: 'Kanit', fontSize: 18.0)),
                                    onPressed: () => showDialog<String>(
                                      context: context,
                                      builder: (BuildContext context) =>
                                          AlertDialog(
